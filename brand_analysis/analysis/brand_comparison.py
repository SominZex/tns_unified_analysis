...
def brand_comparison_analysis(data, selected_brands):
    ...
    brand_comparison = (filtered_data.groupby('brandName')['totalProductPrice']
                        .sum()
    ...
    if chart_type == "Bar Chart":
        fig = px.bar(brand_comparison, x='brandName', y='totalProductPrice', title="Brand Comparison by Sales",
                     labels={'totalProductPrice': 'Sales', 'brandName': 'Brand'}, color_discrete_sequence=[color_scheme])
    elif chart_type == "Pie Chart":
        fig = px.pie(brand_comparison, names='brandName', values='totalProductPrice', title="Brand Comparison by Sales",
                     color_discrete_sequence=[color_scheme])
    elif chart_type == "Box Plot":
        # Box plot requires sales data to be grouped by brand for distribution analysis
        fig = px.box(filtered_data, x='brandName', y='totalProductPrice', title="Brand Sales Distribution",
                     labels={'totalProductPrice': 'Sales', 'brandName': 'Brand'}, color_discrete_sequence=[color_scheme])
